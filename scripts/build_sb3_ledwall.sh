#!/usr/bin/env bash
# Build SB3 master at native LED-wall 4608x1536 (3:1) from v2 clips.
# Mirrors scripts/build_sb2_ledwall.sh — see that file for design notes.
set -euo pipefail
cd "$(dirname "$0")/.."
ROOT="$PWD"

SLATE_PNG="${SLATE_PNG:-/tmp/slate_3to1.png}"
BODY_IN="$ROOT/films/sb3_film.mp4"
COUNTDOWN_MP3="$ROOT/audio/slate_countdown.mp3"
OUT="$ROOT/films/sb3_master.mp4"

[ -f "$SLATE_PNG" ] || { echo "missing slate $SLATE_PNG" >&2; exit 1; }
[ -f "$BODY_IN" ] || { echo "missing $BODY_IN — run assemble_sb.sh 3 first" >&2; exit 1; }

WORK=$(mktemp -d)
SLATE_DURATION=3
COUNTDOWN_START=8.80
...
ffmpeg -y -hide_banner -loglevel error -ss 8 -i "$OUT" -frames:v 1 -update 1 \
  -vf "scale=1920:640" "$ROOT/films/sb3_poster.jpg"
